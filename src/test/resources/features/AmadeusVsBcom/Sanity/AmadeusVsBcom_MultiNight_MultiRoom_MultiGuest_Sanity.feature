@Amadeus_vs_BCOM_Sanity @sanity
Feature: Amadeus vs BCOM Multi Night Multi Room Multi Guest Booking

  Background:
    Given Open Browser and Navigate to HotelBooker
    When user enters username and password
    And user clicks login button
    Given user selects client "Test QA Client(Amadeus)"
    Then selected client should display on header
  Scenario Outline: Amadeus vs BCOM Multi Night Multi Room Multi Guest Virtual Card Booking
    # SHOP Phase
    ...
    And user fills guest information for "<rooms>" rooms with "<guests>" guests
    ...
    Then retrieved booking details should match the original reservation with "<rooms>" rooms and "<guests>" guests
    ...
    Examples:
      ...
      | France  | Paris - Location    | Holiday   | 10 Miles | 45   | 5      | 3     | 6      | Booking.com | Yes        | true    | Bed & Breakfast    | Virtual Card   | Full Cancellation| Booker,Travellers   |
      | Italy   | Rome - Location     | Holiday   | 15 Miles | 60   | 4      | 2     | 5      | Booking.com | No         | false   | Room Only          | Virtual Card   | Cancel by Room   | Agent,Client        |